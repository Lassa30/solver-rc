...
const std::array<Corner, 8> EdgeCornerCube::cornersSolved{{{CornerID::URF, 0},
                                                           ...
                                                           {CornerID::DBL, 0},
                                                           {CornerID::DRB, 0}}};
const std::array<Edge, 12> EdgeCornerCube::edgesSolved{
    {EdgeID::UR, 0, EdgeID::UF, 0, EdgeID::UL, 0, EdgeID::UB, 0,
...
EdgeCornerCube::EdgeCornerCube(const std::array<EdgeID, 12> &ep,
  ...
  for (int cornerIdx = 0; cornerIdx < static_cast<int>(CornerID::COUNT);
       cornerIdx++) {
...
bool EdgeCornerCube::cubeIsValid() { return edgeParity() == cornerParity(); }
...
void EdgeCornerCube::multiplyCorners(const EdgeCornerCube &rhs) {
  ...
  for (int cid = 0; cid < static_cast<int>(CornerID::COUNT); cid++) {
    ...
    orientation = calculateOrientation(orientation_lhs, orientation_rhs);
    corners[cid].position = corners_[cornerIdx].position;
    corners[cid].orientation = orientation;
  }
  corners_ = std::move(corners);
}
...
void EdgeCornerCube::multiplyEdges(const EdgeCornerCube &rhs) {
  ...
  for (int e = 0; e < static_cast<int>(EdgeID::COUNT); e++) {
    int edgeIdx = static_cast<int>(rhs.edges_[e].position);
    edges[e].position = edges_[edgeIdx].position;
    edges[e].orientation =
        (rhs.edges_[e].orientation + edges_[e].orientation) % 2;
  }
  edges_ = std::move(edges);
}
